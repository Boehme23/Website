...
from flask import Flask, render_template, session, request, redirect, url_for, send_file, jsonify  # <-- Ensure 'session' is imported
...
@app.route('/portfolio')
def portfolio():
    # Fetch all coin names for the dropdown
    # Conecta ao banco de dados consolidado
    conn = sqlite3.connect('./Binance/binance_history.db')

    # Query para pegar apenas os símbolos únicos das moedas
    query = "SELECT DISTINCT symbol FROM historico_precos ORDER BY symbol"

    # Busca os dados e transforma em uma lista simples
...
def calculate():
    data = request.json # Expecting { 'BTC': 0.5, 'SOL': 0.5 }
    stats = Portfolio.calculate_portfolio_stats(data)
    return jsonify(stats)

@app.route('/criptos', methods=['GET', 'POST'])
def criptos():
    return render_template('criptos.html')
