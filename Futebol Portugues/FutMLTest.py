...
# ==========================================
# 2.2 PER-LEAGUE ROUND DETECTION (FIXED)
# ==========================================
print("Calculating next round per league...")
# Create a mapping of League -> Next Round
league_round_map = df_history.groupby(COLS['league'])[COLS['match_date']].max() + 1


def assign_round(row):
    league = row[COLS['league']]
    # If the league exists in history, use max+1, otherwise default to 1
    return league_round_map.get(league, 1)
...
# ==========================================
print(f"Training models on {len(features)} features...")
# Random Forest
clf = RandomForestClassifier(n_estimators=500, random_state=23).fit(X, y)
rf_preds = clf.predict(X_new)
rf_probs = clf.predict_proba(X_new).max(axis=1)

# Neural Network
scaler = StandardScaler()
X_scaled = scaler.fit_transform(X)
nn_clf = MLPClassifier(hidden_layer_sizes=(64, 32), max_iter=500, random_state=42).fit(X_scaled, y)
nn_preds = nn_clf.predict(scaler.transform(X_new))
nn_probs = nn_clf.predict_proba(scaler.transform(X_new)).max(axis=1)
...
pd.DataFrame(results_list).to_csv('prediction_results.csv', index=False)
print("Success! Results saved to prediction_results.csv with individual league rounds.")